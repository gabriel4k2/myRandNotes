...
data class NoteGenerationConfig(val instrument: Instrument, val timeIntervalMs: Long, val notes : List<Note>){
    fun toUINoteGenerationConfig() : UINoteGenerationConfig{
        ...
        return UINoteGenerationConfig(instrument = instrument, precision = precision, timeInSeconds = TimeInSeconds(timeInSeconds.toString()), notes = notes)
    }

    companion object {
        val INITIAL_CONFIG = NoteGenerationConfig(instrument = Instrument.INITIAL_INSTRUMENT, timeIntervalMs = 1000, notes = NoteUtils.generateInitialNoteList())
        fun fromUINoteGenerationConfig(uiNoteGenerationConfig: UINoteGenerationConfig): NoteGenerationConfig {
            ...
            return NoteGenerationConfig(instrument = uiNoteGenerationConfig.instrument, timeIntervalMs = timeInMs, notes = uiNoteGenerationConfig.notes)
        }
    }
}
